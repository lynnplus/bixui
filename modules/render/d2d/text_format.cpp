...
#include "core/win32/string_ext.h"
...
D2DTextFormat::D2DTextFormat(IDWriteFactory* factory, uintptr_t scopeId, float density)
    : mFactory(factory), mScopeId(scopeId), mDisplayDensity(density) {
    BIX_UNUSED(mDisplayDensity)
}

void D2DTextFormat::setFontFamily(const std::string& name) {
    if (name.empty() || name == mFontFamilyName) {
        return;
    }
...
void D2DTextFormat::setTextSize(float size) {
    ...
    mTextSize = size;
    if (mLayout) {
        throw_if_fail(mLayout->SetFontSize(mTextSize, mTmpTextRange));
    }
}

void D2DTextFormat::setFontWeight(int weight) {
    if (weight < 1 || weight > 999 || mFontWeight == weight) {
        return;
    }
    mFontWeight = weight;
    if (mLayout) {
        throw_if_fail(mLayout->SetFontWeight(static_cast<DWRITE_FONT_WEIGHT>(mFontWeight), mTmpTextRange));
    }
}

void D2DTextFormat::setWordWrapping(WordWrapping wrap) {
    if (wrap == WordWrapping::Wrap) {
        return;
    }
    mWordWrapping = wrap;
    if (mLayout) {
        throw_if_fail(mLayout->SetWordWrapping(convert_as_D2DWordWrapping(wrap)));
    }
}
...
void D2DTextFormat::setFontStyle(FontStyle style) {
    ...
    mFontStyle = style;
    if (mLayout) {
        throw_if_fail(mLayout->SetFontStyle(convert_as_D2DFontStyle(style), mTmpTextRange));
    }
}
...
bool D2DTextFormat::testCast(uintptr_t scope, long castId) const noexcept {
    if (scope != mScopeId || D2DTextFormat_CAST_ID != castId) {
        return false;
    }
    return true;
}
...
IDWriteTextLayout* D2DTextFormat::layout() const noexcept { return mLayout.get(); }

void D2DTextFormat::create() {
    IDWriteTextFormat* format = nullptr;
    auto hr = mFactory->CreateTextFormat(utf8_to_wstring(mFontFamilyName).c_str(), // Font family name.
                                         ...
                                         mTextSize, utf8_to_wstring(mLocale).c_str(), &format);
    throw_if_fail(hr);
    ...
    hr = mFactory->CreateTextLayout(text.c_str(), numeric_cast<UINT32>(text.size()), mFormat.get(),
                                    numeric_cast<float>(mMaxSize.width), numeric_cast<float>(mMaxSize.height), &layout);
    throw_if_fail(hr);
